...
try
    ...
    initials                        = 'KD';
    starting_point                  = 26;                                  % SD of a temporal gaussian in ms (start high)
    stimulus_radius                 = 60;                                  % Arcmin

    fast                            = 1;                                    % Automatically trigger trials

    %-----Experiment Settings---------------
    % (Keep this consistent across subjects)
    experiment_id                   = '2ndOrd';                              % Used in group filename
...
%         screen_clut = screen_clut/255;
    ...
    for trial=1:total_trials
        ...
        for i = 1:mv_length;
            ...
            gratingn = -gratingp;
            moving_grating{i} = (gratingp.*(pedestal==1)+gratingn.*(pedestal==-1)).*circle*time_gauss(i)+background;
        end
